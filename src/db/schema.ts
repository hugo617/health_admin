...
// 用户状态枚举
export const userStatusEnum = pgEnum('user_status', ['active', 'inactive', 'locked', 'deleted']);

// 角色状态枚举
export const roleStatusEnum = pgEnum('role_status', ['active', 'inactive', 'deleted']);

// 权限状态枚举
export const permissionStatusEnum = pgEnum('permission_status', ['active', 'inactive', 'deleted']);

// 权限类型枚举
export const permissionTypeEnum = pgEnum('permission_type', ['menu', 'page', 'button', 'api', 'data']);

// 数据权限规则类型枚举
export const dataPermissionRuleTypeEnum = pgEnum('data_permission_rule_type', ['all', 'org', 'dept', 'self', 'custom']);

// 登录类型枚举
export const loginTypeEnum = pgEnum('login_type', ['email', 'phone', 'wechat', 'oauth']);

// 设备类型枚举
export const deviceTypeEnum = pgEnum('device_type', ['web', 'mobile', 'desktop']);
...
export const users = pgTable('users', {
  ...
  tenantUsernameUnique: unique('users_tenant_username_unique').on(t.tenantId, t.username),
...
export const permissions = pgTable('permissions', {
  ...
  // 联合唯一约束：同一租户内权限代码唯一
  tenantCodeUnique: unique('permissions_tenant_code_unique').on(t.tenantId, t.code),
  // 联合唯一约束：同一租户内权限名称唯一
  tenantNameUnique: unique('permissions_tenant_name_unique').on(t.tenantId, t.name),
  // 索引
...
export const rolePermissions = pgTable(
  ...
  (t) => ({
    ...
    unq: unique('role_permission_tenant_unique').on(t.tenantId, t.roleId, t.permissionId)
  })
);
...
export const organizations = pgTable('organizations', {
  ...
  parentId: bigint('parent_id', { mode: 'number' }).references(() => organizations.id),
...
export const userOrganizations = pgTable('user_organizations', {
  ...
  uniqueUserOrg: unique('user_organizations_unique').on(t.userId, t.organizationId),
...
// 数据权限规则表
export const dataPermissionRules = pgTable('data_permission_rules', {
  id: bigint('id', { mode: 'number' }).primaryKey().generatedByDefaultAsIdentity(),
  tenantId: bigint('tenant_id', { mode: 'number' }).notNull(),
  name: varchar('name', { length: 200 }).notNull(),
...
// 角色数据权限关联表
export const roleDataPermissions = pgTable('role_data_permissions', {
  id: bigint('id', { mode: 'number' }).primaryKey().generatedByDefaultAsIdentity(),
  roleId: integer('role_id').notNull().references(() => roles.id, { onDelete: 'cascade' }),
  ruleId: bigint('rule_id', { mode: 'number' }).notNull().references(() => dataPermissionRules.id, { onDelete: 'cascade' }),
  grantedAt: timestamp('granted_at').defaultNow(),
  grantedBy: integer('granted_by').references(() => users.id)
}, (t) => ({
  uniqueRoleRule: unique('role_data_permissions_unique').on(t.roleId, t.ruleId),
...
export const userSessions = pgTable('user_sessions', {
  ...
  impersonatorIdIdx: index('idx_user_sessions_impersonator_id').on(t.impersonatorId),
  isActiveIdx: index('idx_user_sessions_is_active').on(t.isActive)
}));

// 用户登录方式表
export const userLoginMethods = pgTable('user_login_methods', {
  id: bigint('id', { mode: 'number' }).primaryKey().generatedByDefaultAsIdentity(),
  userId: integer('user_id').notNull().references(() => users.id, { onDelete: 'cascade' }),
  ...
  uniqueUserLoginType: unique('user_login_methods_unique').on(t.userId, t.loginType, t.identifier),
...
export const dataPermissionRulesRelations = relations(dataPermissionRules, ({ one, many }) => ({
  tenant: one(tenants, {
...
export const userLoginMethodsRelations = relations(userLoginMethods, ({ one }) => ({
  user: one(users, {
...
export type DataPermissionRuleType = typeof dataPermissionRuleTypeEnum.enumValues[number];
