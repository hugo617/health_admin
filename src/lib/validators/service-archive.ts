...
// 客户编号验证
const customerNoSchema = z.string().regex(/^C\d{12}$/, '客户编号格式不正确,应为C+12位数字');
...
const basicInfoSchema = z.object({
  ...
  age: z.string().refine((val) => !val || /^\d{1,3}$/.test(val), '年龄格式不正确').default(''),
  contact: z.string().refine((val) => !val || /^1[3-9]\d{9}$/.test(val), '手机号格式不正确').default(''),
  weight: z.string().refine((val) => !val || /^\d+(\.\d+)?$/.test(val), '体重格式不正确').default(''),
  height: z.string().refine((val) => !val || /^\d+(\.\d+)?$/.test(val), '身高格式不正确').default(''),
});
...
const healthHistorySchema = z.object({
  surgeryHistory: z.object({
    ...
    time: z.string().refine((val) => !val || /^\d{4}-\d{2}-\d{2}$/.test(val), '日期格式不正确').default(''),
  }),
...
const signatureSchema = z.object({
  ...
  signature: z.string().default('').refine((val) => {
    if (!val) return true;
    // 验证Base64格式
    return val.startsWith('data:image/') && val.includes('base64,');
  }, '签名格式不正确,应为Base64格式图片'),
  date: z.string().refine((val) => !val || /^\d{4}-\d{2}-\d{2}$/.test(val), '日期格式不正确').default(''),
});
...
// 完整的档案验证
export const createServiceArchiveSchema = z.object({
  customerNo: customerNoSchema.optional(),
  channels: channelsSchema,
  basicInfo: basicInfoSchema,
  healthHistory: healthHistorySchema,
  subjectiveDemand: z.string().max(500, '主观诉求不能超过500个字符').default(''),
...
// 更新档案验证(所有字段可选)
export const updateServiceArchiveSchema = createServiceArchiveSchema.partial();

// 查询参数验证
export const serviceArchiveQuerySchema = z.object({
