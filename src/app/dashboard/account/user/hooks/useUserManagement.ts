...
export function useUserManagement() {
    ...
    try {
      ...
      if ((res.code === 0 && res.data) || res.success) {
        ...
        setPagination({
          ...
          totalPages: pager?.totalPages || Math.ceil((pager?.total || userData.length) / (filters.limit || 10))
        });
        ...
        if (!statistics) {
          ...
          setStatistics({
            overview: {
              ...
              activeRate: userData.length > 0 ? Math.round((activeCount / userData.length) * 100 * 100) / 100 : 0
            },
            engagement: {
              recentLogins: userData.filter(u => u.lastLoginAt).length,
              recentLoginRate: userData.length > 0 ? Math.round((userData.filter(u => u.lastLoginAt).length / userData.length) * 100 * 100) / 100 : 0
            },
            growth: {
              thisMonth: userData.filter(u => {
                const createdThisMonth = new Date(u.createdAt);
                return createdThisMonth.getMonth() === new Date().getMonth() &&
   ...
   * 批量操作用户
   */
  const batchOperateUsers = useCallback(async (
    operation: string,
    userIds: number[],
    data?: any
  ): Promise<boolean> => {
    try {
      const res = await UserAPI.batchOperateUsers(operation as any, userIds, data);
      if (res.success) {
   ...
   * 终止用户会话
   */
  const terminateUserSessions = useCallback(async (
    id: number,
