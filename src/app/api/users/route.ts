import { db } from '@/db';
import { roles, users, tenants, userOrganizations, organizations } from '@/db/schema';
import bcrypt from 'bcryptjs';
import { eq, like, and, gte, lte, sql, inArray, isNull, desc, asc } from 'drizzle-orm';
...
export async function GET(request: Request) {
  try {
    ...
    console.log('查询参数:', { page, limit, search, status, sortBy, sortOrder });
    ...
    if (sortBy === 'createdAt') {
      ...
      orderByClause = sortOrder === 'desc' ? desc(users.username) : asc(users.username);
...
async function getUserStatistics(tenantId?: number) {
  ...
  const [
    ...
    db.select({ count: sql`count(*)` }).from(users).where(
      and(
        ...conditions,
        gte(users.lastLoginAt, new Date(Date.now() - 30 * 24 * 60 * 60 * 1000))
      )
    ),
...
export async function POST(request: Request) {
  ...
  try {
    ...
    // 确定租户ID
    const finalTenantId = currentUser?.isSuperAdmin && tenantId ?
      Number(tenantId) :
